...
async function loadAllUser(page, param) {
    ...
    for (i = 0; i < listUser.length; i++) {
        ...
        }else if (listUser[i].authorities.name == 'ROLE_STAFF') {
            btn = '<td class="sticky-col"></td>'
        }
        main += `<tr>
                    <td>${listUser[i].id}</td>
...
async function addAdmin() {
    ...
    var quyenInput = document.getElementById("quyen");
    ...
    if (!phoneRegex.test(phone)) {
        toastr.warning("Số điện thoại không hợp lệ. Vui lòng nhập đúng 10 chữ số.");
        return;
    }
    ...
    if (password.length < 5) {
        toastr.warning("Mật khẩu phải có ít nhất 5 ký tự.");
        return;
    }
    ...
    if (!repassword) {
        toastr.warning("Vui lòng nhập lại mật khẩu.");
        return;
    }
